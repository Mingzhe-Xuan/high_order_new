...
from e3nn.o3 import FullyConnectedTensorProduct, ElementwiseTensorProduct
import math
import torch
...
_idx_data = torch.load(
    os.path.join(os.path.dirname(__file__), "z_rot_indices_lmax12.pt"),
    weights_only=False,
)
...
def build_z_rot_multi(angle_stack, mask, freq, reversed_inds, offsets, sizes):
    ...
    M_total[:, global_row[~overlap_mask], global_col_anti[~overlap_mask]] = sin_val[
        idx_l[~overlap_mask], :, idx_row[~overlap_mask]
    ].transpose(0, 1)
...
def batch_wigner_D(l_max, alpha, beta, gamma, _Jd):
    ...
    Xa, Xb, Xc = build_z_rot_multi(
        angle_stack, mask, freq, reversed_inds, offsets, sizes
    )
        ...
        for m in range(self.irreps_out.lmax + 1):
            ...
            if m == 0:
                if self.front and self.radial_emb:
                    out[:, self.m_out_mask[m]] += self.fc_m0(
                        x_[:, self.m_in_mask[m]] * radial_weight.squeeze(1)
                    )
                elif self.radial_emb:
                    out[:, self.m_out_mask[m]] += self.fc_m0(
                        x_[:, self.m_in_mask[m]]
                    ) * radial_weight.squeeze(1)
                else:
                ...
                final_addition = (
                    linear_output.transpose(1, 2).contiguous().reshape(n, -1)
                )
